...
void PriorKnowledge::init_variant(const Board& bd)
{
    ...
    {
        Float temperature = 0.84f;
        m_gamma_point_other = exp(0.219f / temperature);
        m_gamma_point_opp_attach_or_nb = exp(1.944f / temperature);
        m_gamma_point_second_color_attach = 1; // unused
        m_gamma_adj_connect = exp(-0.025f / temperature);
        m_gamma_adj_occupied_other = exp(-0.024f / temperature);
        m_gamma_adj_forbidden_other = 1; // unused
        m_gamma_adj_own_attach = exp(-0.527f / temperature);
        m_gamma_adj_nonforbidden = exp(0.229f / temperature);
        m_gamma_attach_to_play = exp(-0.213f / temperature);
        m_gamma_attach_forbidden_other = exp(-0.589f / temperature);
        m_gamma_attach_nonforbidden[0] = 1; // unused
        m_gamma_attach_nonforbidden[1] = exp(0.075f / temperature);
        m_gamma_attach_nonforbidden[2] = exp(0.108f / temperature);
        m_gamma_attach_nonforbidden[3] = exp(0.040f / temperature);
        m_gamma_attach_nonforbidden[4] = exp(0.179f / temperature);
        m_gamma_attach_nonforbidden[5] = 1; // unused
        m_gamma_attach_nonforbidden[6] = 1; // unused
        m_gamma_attach_second_color = 1; // unused
        m_gamma_local = exp(0.156f / temperature);
        gamma_piece_score_0 = exp(0.781f / temperature);
        gamma_piece_score_1 = 1; // unused
        gamma_piece_score_2 = exp(-1.381f / temperature);
        gamma_piece_score_3 = exp(-0.755f / temperature);
        gamma_piece_score_4 = exp(0.340f / temperature);
        gamma_piece_score_5 = exp(0.988f / temperature);
        gamma_piece_score_6 = 1; // unused
    }
    ...
    else if (piece_set == PieceSet::nexos)
    {
        Float temperature = 0.84f;
        // Tuned for nexos_2
        m_gamma_point_other = exp(0.544f / temperature);
        m_gamma_point_opp_attach_or_nb = exp(1.428f / temperature);
        m_gamma_point_second_color_attach = exp(0.173f / temperature);
        m_gamma_adj_connect = exp(0.125f / temperature);
        m_gamma_adj_occupied_other = exp(0.033f / temperature);
        m_gamma_adj_forbidden_other = 1; // unused
        m_gamma_adj_own_attach = exp(-0.213f / temperature);
        m_gamma_adj_nonforbidden = exp(0.068f / temperature);
        m_gamma_attach_to_play = exp(0.002f / temperature);
        m_gamma_attach_forbidden_other = exp(-0.049f / temperature);
        m_gamma_attach_nonforbidden[0] = 1; // unused
        m_gamma_attach_nonforbidden[1] = exp(0.181f / temperature);
        m_gamma_attach_nonforbidden[2] = exp(-0.002f / temperature);
        m_gamma_attach_nonforbidden[3] = exp(0.002f / temperature);
        m_gamma_attach_nonforbidden[4] = exp(-0.113f / temperature);
        m_gamma_attach_nonforbidden[5] = exp(-0.082f / temperature);
        m_gamma_attach_nonforbidden[6] = exp(0.095f / temperature);
        m_gamma_attach_second_color = exp(-0.015f / temperature);
        m_gamma_local = exp(1.121f / temperature);
        gamma_piece_score_0 = 1; // unused
        gamma_piece_score_1 = exp(0.106f / temperature);
        gamma_piece_score_2 = exp(-0.318f / temperature);
        gamma_piece_score_3 = exp(-0.427f / temperature);
        gamma_piece_score_4 = exp(0.643f / temperature);
    ...
    else if (geometry_type == GeometryType::callisto)
    {
        ...
        m_gamma_point_other = exp(0.206f / temperature);
        m_gamma_point_opp_attach_or_nb = exp(1.917f / temperature);
        m_gamma_point_second_color_attach = exp(0.052f / temperature);
        m_gamma_adj_connect = exp(0.160f / temperature);
        m_gamma_adj_occupied_other = exp(-0.022f / temperature);
        m_gamma_adj_forbidden_other = 1; // unused
        m_gamma_adj_own_attach = exp(-0.439f / temperature);
        m_gamma_adj_nonforbidden = exp(0.125f / temperature);
        m_gamma_attach_to_play = exp(-0.179f / temperature);
        m_gamma_attach_forbidden_other = exp(-0.487f / temperature);
        m_gamma_attach_nonforbidden[0] = 1; // unused
        m_gamma_attach_nonforbidden[1] = exp(0.264f / temperature);
        m_gamma_attach_nonforbidden[2] = exp(0.147f / temperature);
        m_gamma_attach_nonforbidden[3] = exp(0.036f / temperature);
        m_gamma_attach_nonforbidden[4] = exp(0.060f / temperature);
        m_gamma_attach_nonforbidden[5] = 1; // unused
        m_gamma_attach_nonforbidden[6] = 1; // unused
        m_gamma_attach_second_color = exp(-0.057f / temperature);
        m_gamma_local = exp(0.256f / temperature);
        gamma_piece_score_0 = exp(0.682f / temperature);
        gamma_piece_score_1 = 1; // unused
        gamma_piece_score_2 = exp(-1.291f / temperature);
        gamma_piece_score_3 = exp(-0.679f / temperature);
        gamma_piece_score_4 = exp(0.366f / temperature);
        gamma_piece_score_5 = exp(0.941f / temperature);
        gamma_piece_score_6 = 1; // unused
    }
